...
def main() -> None:
    ...
    with st.sidebar:
        ...
        st.title(gettext("settings"))
        st.caption(gettext("Configure the settings used to create your prediction explanations"))
    ...
    with title_container:
        ...
        new_email, multiple_emails, outcome_information = st.tabs(
            [gettext("新しい予測"), gettext("バッチ予測"), gettext("予測結果の詳細")]
        )
            ...
            with st.spinner(
                ...
                with prediction_response_container:
                    ...
                    prediction_info_expander = st.expander(
                        gettext("Information for {selected_record}!").format(
                ...
                with email_draft_container:
                    ...
                    if predicted_label == app_settings.no_text_gen_label:
                        ...
                        st.subheader(gettext("生成AIによる予測の説明:"))
    ...
    with multiple_emails:
        ...
        run = run_button.button(gettext("Generate Predictions"))
        if run and csv is not None:
            ...
            with st.spinner(
                ...
                status_bar.success(
                    gettext("Predictions have been made! Generating predictions...")
                )

            # st.session_state.bulk_prediction_results = predictions

            with st.spinner(
                gettext("Generating prediction explanations for {count} records...").format(count=count)
            ):
            ...
            status_bar.success(
                gettext(
                    "Finished! All {count} texts have been drafted and results have been saved."
                ).format(count=count)
            )

            st.dataframe(emails)
        elif run:
            status_bar.error(gettext("Please upload a csv file to generate texts."))  # type: ignore[used-before-def]
